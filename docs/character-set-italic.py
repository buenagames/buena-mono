...
# Name the styled instance from fvar (falls back to "Italic")
FAMILY_NAME = ttFont["name"].getDebugName(16) or ttFont["name"].getDebugName(1)
STYLE_NAME = "Italic"
for inst in ttFont["fvar"].instances:
    if inst.coordinates == VARIATIONS:
        STYLE_NAME = ttFont["name"].getDebugName(inst.subfamilyNameID)
        break
FONT_NAME = f"{FAMILY_NAME} {STYLE_NAME}"
...
BIG_TEXT_FONT_SIZE = (WIDTH - MARGIN * 2) / (LONGEST * ADVANCE / UPM)
...
def draw_main_text():
    ...
    for i, line in enumerate(LINES):
        y = BIG_TEXT_BOTTOM_MARGIN - (MARGIN * LEADING * i)
