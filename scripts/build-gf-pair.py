...
def build_instance(input_path, config, slnt, italic):
    ...
    if italic:
        ...
        # hhea caret slope must match the italic angle (instancing the slnt
        # axis leaves the wrong sign behind): rise=upem, run=tan(-angle)*upem
        hhea, upem = font["hhea"], head.unitsPerEm
        hhea.caretSlopeRise = upem
        hhea.caretSlopeRun = round(math.tan(math.radians(-float(slnt))) * upem)
